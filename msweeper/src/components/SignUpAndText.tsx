...
const SignUpAndText : React.FC<SignUpAndTextProps> = ({auth, user, setUser, uid, setUid}) => {
  ...
  const handleOnClickSignOut = () => {
    ...
    fetch(`http://localhost:5001/minesweeper-9667e/us-central1/leaderboard/delete`, {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      fetch(`http://localhost:5001/minesweeper-9667e/us-central1/leaderboard/login`, {
      ...
      .then(async (result) => {
        if (result) {
          ...
          // fetch(`${process.env.REACT_APP_API_URL!}/signup`, {
            fetch(`http://localhost:5001/minesweeper-9667e/us-central1/leaderboard/signup`, {
    ...
    <>
      {user ? (
        ...
        <div className='fixed h-[105vh] w-[100vw] bg-[#00000070] z-[1000] bottom-[0.5vh] top-[0.5vh]'>
          <div className='fixed h-[90vh] w-[50vw] bg-[#646464] 
          ...
          <div className='flex flex-row  justify-between mx-[1vw] my-[1vh] items-center '> 
            <h2 className='text-[1.95vw]'>{user.name}'s Profile Stats 📜</h2>
            <CgCloseR className='cursor-pointer hover:text-[#bd0404] text-[#ff3c2e]' size={"3vw"} onClick={() => setShow(false)}></CgCloseR>
          </div>
          ...
          <div className='flex flex-col justify-between h-[87%]'>
            ...
            <div className='flex flex-row justify-center  overflow-y-auto'>
              ...
              <div className='flex flex-col my-[1vh] mx-[1vw] justify-between text-center text-[2.8vw] w-[30vw]'>
                    ...
                    <h2 className="font-bold text-[3vw]"><u>Best time</u> ⌛</h2>
                      
                    <h3>Easy: {user.BestTimeEasy == 1000 ? "N/A" : user.BestTimeEasy}</h3>
                    <h3>Medium: {user.BestTimeMedium == 1000 ? "N/A" : user.BestTimeMedium}</h3>
                    <h3>Hard: {user.BestTimeHard == 1000 ? "N/A" : user.BestTimeHard}</h3>
                    <br />
                    <h2 className="font-bold text-[3vw]" ><u>Gold Coins</u> 💰 </h2>
                    <h3>{user.goldCoins} coins</h3>
                    <br />
                  
                      <h2  className="font-bold text-[3vw]"><u>Game Stats</u> 📈</h2>
                      <h3>Total Game: {user.totalGame} </h3>
                      <h3>Total Time: {user.totalTime}</h3>
                      <h3>Total Win: {user.easyWin + user.mediumWin + user.hardWin}</h3>
